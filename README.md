...
| `GLINER_PII_URL`  | *(empty)*                     | When set, enables the `gliner_pii` detector (DETECTOR_MODE=…,gliner_pii). HTTP base URL of a [gliner-pii-service](services/gliner_pii/README.md) container. There is no in-process variant — empty + `gliner_pii` in DETECTOR_MODE crashes loud at boot. **Experimental.** |
| `GLINER_PII_TIMEOUT_S` | `30`                     | Per-call timeout (seconds) on the gliner-pii HTTP requests.                                                                                                                                                                  |
| `GLINER_PII_FAIL_CLOSED` | `true`                 | Block requests when the gliner-pii detector errors (mirrors `LLM_FAIL_CLOSED` / `PRIVACY_FILTER_FAIL_CLOSED`). Independent flag.                                                                                              |
...
| `GLINER_PII_MAX_CONCURRENCY` | `10`              | Semaphore on in-flight gliner-pii calls. Independent of LLM/PF caps. Surfaced as `gliner_pii_in_flight`/`gliner_pii_max_concurrency` on `/health`.                                                                          |
...
For everything else — repo layout, the registry-driven architecture,
the launcher's internals, and the canonical "adding a new detector"
walkthrough — see [DEVELOPMENT.md](DEVELOPMENT.md).
