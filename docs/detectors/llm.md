...
| `LLM_CACHE_BACKEND` | `memory` | `memory` (process-local LRU) or `redis` (shared store). The redis backend requires the cross-cutting `CACHE_REDIS_URL` and `CACHE_SALT` (see [configuration → Detector cache](../configuration.md#detector-cache-cross-cutting)) and the `cache-redis` install extra. |
