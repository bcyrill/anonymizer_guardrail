...
| `LLM_CACHE_MAX_SIZE` | `0` | LRU cap on the LLM detector's result cache (memory backend only). `0` disables caching (default). When enabled, repeat calls with the same `(text, llm_model, llm_prompt)` skip the LLM round-trip. See [operations → Detector result caching](../operations.md#detector-result-caching) for the trade-offs. |
| `LLM_CACHE_BACKEND` | `memory` | `memory` (process-local LRU) or `redis` (shared store). The redis backend requires the central `CACHE_REDIS_URL` and the `cache-redis` install extra. See [operations → Detector result caching](../operations.md#detector-result-caching). |
| `LLM_CACHE_TTL_S` | `600` | Per-key TTL for redis-backed entries, reset on every cache write. Memory backend ignores this knob (LRU eviction is bounded by `cache_max_size`). |
