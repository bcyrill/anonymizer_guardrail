...
| `GLINER_PII_CACHE_MAX_SIZE` | `0` | LRU cap on the gliner-pii detector's result cache (memory backend only). `0` disables caching (default). When enabled, repeat calls with the same `(text, labels, threshold)` skip the remote gliner round-trip. Per-call `labels` order is part of the cache key — different orderings get different slots. See [operations → Detector result caching](../operations.md#detector-result-caching) for the trade-offs. |
| `GLINER_PII_CACHE_BACKEND` | `memory` | `memory` (process-local LRU) or `redis` (shared store). The redis backend requires the central `CACHE_REDIS_URL` and the `cache-redis` install extra. |
| `GLINER_PII_CACHE_TTL_S` | `600` | Per-key TTL for redis-backed entries, reset on every cache write. Memory backend ignores this knob. |
