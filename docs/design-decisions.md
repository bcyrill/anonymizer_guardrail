...
## Response side runs deanonymize only — no detection, no cache pre-warming

**Considered:** running detector layers on the response side
(`input_type="response"`) in addition to the existing vault lookup,
either to (a) anonymize *new* PII the model emits in its reply or
(b) pre-warm the per-detector result cache so the next turn's
request doesn't pay for re-detecting the assistant message.

The natural shape: in `main.py`, `input_type="response"` would
call `pipeline.anonymize` *before* (or instead of) `deanonymize`,
storing the assistant message's matches in the cache. On the next
turn, when LiteLLM replays that same assistant message inside
`req.texts`, it would hit cache and skip the round-trip.

**The pull:**

- Closes a round-trip per turn. The detector cache today gives
  cache hits on every prior *user* message and the system prompt,
  but **always misses on the most recent assistant message** —
  because that message was never put through detection. Pre-
  warming would change "two new detection calls per round-trip"
  (one user, one assistant on first sight) to one (just the new
  user message).
- Catches new PII the model emits. Today's response path is a
  pure substring-replace from the vault keyed by `call_id`. If
  the model invents an email address or person name that wasn't
  in the request, that goes back to the user untouched. Detection
  on the response side would close this gap.
...
- **The "anonymize new model output" problem deserves explicit
  scoping, not a side effect.** Whether to redact PII the model
  generates is a real product question (does it block the
  response, modify it, only log it?). Folding that decision into
  a cache-warming optimization would commit to an answer
  implicitly. Keep response-side anonymization as a separate
  design concern with its own decision when a use case actually
  needs it.
- **Cache hit rate is already good for the bulk of conversation
  history.** Steady-state cost is ~2 detection calls per
  round-trip (one for the new user message at first sight, one
  for the new assistant message at first sight) regardless of
  conversation length. The wasted call on the assistant message
  is bounded — it's a constant per turn, not a multiplier on
  history length. The cache already wins where it matters most.
...
- The detector cache learns about an assistant message *one turn
  late*: turn N's assistant reply is processed when it appears
  inside turn N+1's request, after which it's cached for turn
  N+2 and beyond.
...
- A use case shows up where the model is expected to emit PII
  ...
  its own right, and the cache pre-warming falls out of solving
  it.
- A workload shows that the "assistant message at first sight"
  cache miss is a measurable share of detection cost — tracked
  via `<prefix>_cache_hits` / `<prefix>_cache_misses` on
  `/health`. If misses dominate even after several turns of
  conversation, the steady-state assumption above isn't holding.
