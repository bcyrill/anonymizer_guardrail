...
| `STREAMING_FAKER_MODE` | `disabled` | Streaming-mode behaviour under `USE_FAKER=true`. `disabled` preserves streaming UX but lets partial Faker surrogates that straddle chunk boundaries leak to the client (the default; matches today's behaviour). `buffer` returns `WAIT` on every non-final response chunk when `use_faker` is active, deferring substitution to the `is_final=True` call against the full assembled text — correct end-to-end but eliminates streaming UX (time-to-first-token becomes time-to-last-token). Has no effect when `use_faker` is `false` (opaque mode streams correctly via the existing partial-surrogate `WAIT` predicate). See [limitations → Faker mode trades streaming UX for correctness](limitations.md#faker-mode-trades-streaming-ux-for-correctness-opt-in-flag). |
